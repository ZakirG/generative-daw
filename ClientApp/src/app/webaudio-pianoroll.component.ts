...
@Component({
  selector: 'webaudio-pianoroll-component',
  templateUrl: './webaudio-pianoroll.component.html',
...
export class WebAudioPianoRollComponent implements AfterViewInit {
    ...
    colnote : string;
    colnotesel :string;
    colnoteborder :string;
    colnoteselborder : string;
    colrulerbg :string;
    colrulerfg :string;
    colrulerborder:string;
    colselarea :string;
    bgsrc :string;
    cursorsrc :string;
    cursoroffset : number;
    markstartsrc : string;
    markstartoffset :number;
    markendsrc :string;
    markendoffset :number;
    kbsrc :string;
    kbwidth : number;
    loop : number;
    preload : number;
    ...
    @ViewChild('wacWrapperEl', { read: ElementRef, static: true }) wacWrapper:ElementRef; 
    ...
    constructor (
        ...
        const plist = {
            width:              {type:Number, value:820, observer:'layout'},
            height:             {type:Number, value:620, observer:'layout'},
            ...
            yrange:             {type:Number, value:48, observer:'layout'},
            ...
            xscroll:            {type:Number, value:1},
            yscroll:            {type:Number, value:1},
    ...
    ready(){
        this.elem = this.wacWrapper.nativeElement.childNodes[0];
        ...
        for(let i=l-1;i>=0;--i){
            ...
            if(typeof(n)!="undefined" && n!=i){
                if(t<=ev.t && t+g>=ev.t+ev.g){
                    ...
                    this.addNote(t+g,ev.n,ev.t+ev.g-t-g,this.defvelo, null);
                    ev.g=t-ev.t;
                }
            }
        }
                ...
                if(this.dragging.ev){
                    ...
                    for(let i = list.length - 1; i >= 0; --i){
                        ...
                        if(this.editmove=="dragmono")
                            this.delAreaNote(ev.t,ev.g, null);
                    }
    ...
    editGridDown(pos){
        ...
        if(ht.m=="n"){
            ...
            this.addNote(pt,ht.n|0,1,this.defvelo, null);
        ...
        if(this.dragging.o=="G"){
            ...
            case "1":
                ...
                if(ht.m=="s"){
                    ...
                    this.addNote(px,ht.n|0,1,this.defvelo, null);
                }
                break;
            case "0":
                if(ht.m=="n")
...
//            this.mousemove(e);
    ...
    layout(){
        ...
        const bodystyle = this.wacWrapper.nativeElement.style;
        if(this.bgsrc)
    ...
    sendEvent(ev){
        ...
        if(this.wacWrapper) {
            this.wacWrapper.nativeElement.dispatchEvent(event);
        }
    }

    getAttr(n,def){
        if (this.wacWrapper) {
            let v=this.wacWrapper.nativeElement.getAttribute(n);
